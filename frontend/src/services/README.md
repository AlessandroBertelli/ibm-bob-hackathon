...
| status.service.ts | `getServiceStatus()` — feeds the landing-page status dots from `/api/system/status`. |
| account.service.ts | `deleteAccount()` — calls `DELETE /api/auth/account` and signs out locally. |
